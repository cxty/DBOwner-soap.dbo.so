...
class DBOModel
{
    ...
    public function __construct($config=array())
    {
		 ...
		 if(!defined('is_load_DBOConfig'))
		  ...
		  $this->config=is_array($config)?array_merge(DBOConfig::get('DB'),$config):DBOConfig::get('DB');//参数配置	
	...
	public function connect()
	{
		 ...
		 if(!is_object($this->db))
		 {
			  ...
			  if(!file_exists(dirname(__FILE__).'/db/'.$db_type.'.class.php'))
			  {
					$this->error($db_type.'数据库类型没有驱动');
			  }
			  require_once(dirname(__FILE__).'/db/'.$db_type.'.class.php');//加载数据库类
    ...
    public function queryArray($sqlArray)
    {
    	...
    	$this->connect();
    	return $query=$this->db->queryArray($sqlArray);
    }
