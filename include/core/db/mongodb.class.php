...
class mongo_db {
	...
	// 连接数据库
	public function connect($dbhost, $dbuser, $dbpw, $_dbname = '', $AUTH = false) {
		...
		if ($this->dbauth) {
			...
			foreach ( $dbs as $_dbname ) {

				$ret = $this->link->selectDB ( $_dbname )->authenticate ( $dbuser, $dbpw );
		...
		if ($_dbname) {
			$this->select_db ( $_dbname );
		}

	}
	// 选择数据库
	public function select_db($dbname) {
		$_db = $this->link->selectDB ( $dbname );

		if($_db){
			return $_db;
		}else{
			return $this->link->{$dbname};
		}
	}
	public function table($tablename) {
		return $this->setcollection ( $tablename );
	}
	 ...
	 * 返回知道字段
	 */
	public function field($_field = array()){
		foreach ( $_field as $k=>$v ) {
			$this->_results [$k] = ($v==1||$v=='1'||$v==true||$v=='true')?1:0;
		}
		return $this;
	}
	/**
	...
	public function slice($attr, $subOffset, $subLimit) {
		$this->_results [$attr] ['$slice'] = array (
				intval ( $subOffset ),
				intval ( $subLimit )
		);
		return $this;
	...
	public function insert(array $attrs, $safe = false) {
		...
		$bool = $this->_collection->insert ( $attrs, array (
				"safe" => $safe
		) );
		if ($bool) {
	...
	public function upsert(array $obj) {
		return $this->_collection->update ( $this->criteria (), $obj, array (
				"upsert" => true,
				"multiple" => true
		) );
	}
	...
	public function update(array $obj,array $option){
		return $this->_collection->update ( $this->criteria (), $obj, $option );
	}
	/**
	 * 批量插入一组新的数据
	 *
