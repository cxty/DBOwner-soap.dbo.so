...
class mysql {
	...
	//innodb引擎关闭自动事务方式提交
	public function queryNOAUTOCOMMIT($sql)
	{
	...
	public function queryArray($sqlArray)
	{
		...
		if($sqlArray){
			...
			for($i=0;$i<count($sqlArray);$i++){
				if(!($query = mysql_query($sqlArray[$i], $this->link)))
				{
					$isOK = false;
					$errorSQL = 'MySQL Query Error'. $sqlArray[$i];
					break;
				}
			}
			if($isOK){
				...
				$this->halt('MySQL Query Error', $errorSQL);
			}
			mysql_query("END");
