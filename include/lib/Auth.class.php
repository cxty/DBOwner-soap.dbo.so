...
// 权限认证类
class Auth {
	public static $model = NULL; // 数据库模型
	...
	static public function init($config = array()) {
		...
		// 不需要认证的模块，对后台认证有效
		self::$config ['AUTH_LOGIN_NO'] = isset ( $config ['AUTH_LOGIN_NO'] ) ? $config ['AUTH_LOGIN_NO'] : array (
				'index' => array (
						'login',
						'verify' 
				),
	 ...
	 */
	static public function WriteCookie($key, $value, $exp=0) {
		if ($_COOKIE) {
			SetCookie (self::$config ['AUTH_SESSION_PREFIX'].$key, $value, $exp );
		}
	}
	/**
	 * 读Cookies
	...
	static public function ReadCookie($key) {
		if ($_COOKIE) {
			return $_COOKIE[self::$config ['AUTH_SESSION_PREFIX'].$key];
		}
	}
	// 设置认证用户组id
	...
	static public function getModule($model, $config = array(), $module_path = '', $module_suffix = '') {
		...
		$group_field_id = self::$config ['AUTH_TABLE'] ['group'] ['field'] ['id']; // 用户组表
		                                                                     // id字段
		$group_field_power = self::$config ['AUTH_TABLE'] ['group'] ['field'] ['power']; // 用户组表
		                                                                           // 权限power字段
		
		$resource_field_id = self::$config ['AUTH_TABLE'] ['resource'] ['field'] ['id']; // 资源表
		                                                                           // id字段
		$resource_field_pid = self::$config ['AUTH_TABLE'] ['resource'] ['field'] ['pid']; // 资源表
		                                                                             // pid字段
		$resource_field_operate = self::$config ['AUTH_TABLE'] ['resource'] ['field'] ['operate']; // 资源表
		                                                                                     // operate字段
		                                                                                     // 如果没有用户组和资源数据表，则自动创建
